...
\ProvidesFileSVN{$Id: chronos.tex 9884 2023-11-21 19:38:19Z cfrees $}[v0.8 \revinfo]
\GetFileInfoSVN*
...
\svnid{$Id: chronos.tex 9884 2023-11-21 19:38:19Z cfrees $}
...
\end{chronoscode}
The adjustments are required to ensure that the tapered part is not counted when \enviro{chronos} calculates how much of the total \keyname[idx pre=timeline,link pre=timeline/,idx font pre=\ttfamily,type=dimkey]{timeline width} is available to represent time.
  ...
  blues below/.style={%
    ...
    timeline={%
      ...
      conditional timeline arrow={%
        ...
        chronos tikz+={\path (chronos post) -- ++(3pt+2.1\timelineht,0pt) (chronos pre) -- ++(-\timelineht/3,0pt);},
      }{},
...
\end{chronoscode}
This definition is chosen because it is the most technically complex example.
...
\subsection{Timeline Arrow}\label{subsec:timeline-arrow}
% BEGIN subsec:timeline-arrow
...
\begin{itemize}
  ...
  \Pkgname{chronos} adjusts automatically for \keysname[type=dimkey,idx pre=timeline,link pre=timeline/]{timeline margin,timeline era margin}, but \styleslabelname{} are responsible for other adjustments.
...
\Pkgname{chronos} will switch the \keylabelname{} path to \texttt{/chronos/} prior to using the list, but the \texttt{timeline} prefix must be specified if required.
    ...
    \pgfqkeys{/chronos}{%
      ...
      do timeline arrow/.add code={%
        \ifchronostimelinearrow 
          \tikzset{/chronos/timeline/timeline@arrow}%
        \else
          \tikzset{/chronos/timeline/no@timeline@arrow}%
        \fi
      },
    }%
  },
...
\begin{itemize}
  ...
  \begin{table}
    \centering
    \caption{Public names for \pkgname[no link]{chronos} internal macros defined locally within the \conceptname[no idx,type=concept]{\meta{timeline specification}}.}\label{tab:enviro}
    \begin{tabular}{ll}
      \toprule
