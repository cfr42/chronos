...
\ProvidesFileSVN{$Id: chronos.tex 9848 2023-10-28 03:36:17Z cfrees $}[v0.8 \revinfo]
\GetFileInfoSVN*
...
\svnid{$Id: chronos.tex 9848 2023-10-28 03:36:17Z cfrees $}
...
{% addaswyd o doctools t18
  ...
  \clist_map_inline:nn { #2 }
  {
    ...
    {
      \par
      \textcolor{WildStrawberry}{\l_tmpa_tl}
    }
    \par
  }
...
{% #1 conjunction
%   \bool_if:NT \g__chronosdoc_enwau_ll_bool
%   {
%     \seq_set_map:NNn \l__chronosdoc_tmpb_seq \l__chronosdoc_enwau_seq
%     {
%       ##1 s
%     }
%     \seq_set_eq:NN \l__chronosdoc_enwau_seq \l__chronosdoc_tmpb_seq
%   }
...
\NewDocumentCommand \keyname { O {} m }
{
  \group_begin:
    \bool_set_false:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\ttfamily, link,  #1 } { #2 }
  \group_end:
}
\NewDocumentCommand \keysname { O {} m }
{
  \group_begin:
    \bool_set_false:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\ttfamily, lluosog, link,  #1 } { #2 }
  \group_end:
}
\NewDocumentCommand \conceptname { O {} m }
{
  \group_begin:
    \bool_set_false:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\sffamily, type=element,  #1 } { #2 }
  \group_end:
}
\NewDocumentCommand \conceptsname { O {} m }
{
  \group_begin:
    \bool_set_false:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\sffamily, type=element, lluosog,  #1 } { #2 }
  \group_end:
}
\NewDocumentCommand \pkgname { O {} m }
{
  \group_begin:
    \bool_set_false:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\sffamily, no link, type=pkg,  #1 } { #2 }
  \group_end:
}
\NewDocumentCommand \Keyname { O {} m }
{
...
\NewDocumentCommand \Keysname { O {} m }
{
  \group_begin:
    \bool_set_true:N \l__chronosdoc_uc_bool
    \__chronosdoc_name:nn { font=\ttfamily, link, lluosog, link,  #1 } { #2 }
  \group_end:
}
...
%%% colors
  ...
  keywordstyle=\color{WildStrawberry}\bfseries,%\color{doc@keywordcolor}
...
\begin{figure}[!b]
  ...
  \begin{adjustwidth}{-3em}{-3em}
    ...
    \mmzset{disable}% galluogi hyperlinks
      ...
      \foreach \i/\j/\k  in {split/17.5pt/5pt,dformats2/22.5pt/10pt,noeth/27.5pt/15pt,hol/32.5pt/20pt}
      {
      ...
      \foreach \i/\j/\k/\l/\m in {%
        zero/15pt/5pt/5pt/180,%
        dformats/13pt/7.5pt/7pt/202,
        lol/11pt/10pt/9pt/225,
        dcentric/9pt/12.5pt/11pt/247,
        siml/7pt/15pt/13pt/270%
      }
      {
        \coordinate (a) at ([xshift=\j]chronos year 2020);
      ...
      \foreach \i/\j/\k/\l in {%
        second/5pt/17.5pt/west,
        first/4pt/20pt/south west,
        wphil/2pt/22.5pt/south,
        wphildev/0pt/25pt/south east,
...
The circular \conceptname[link,no idx]{connector} on the \conceptname[type=tag,no idx,link]{event}'s \conceptname[link,idx pre=event,idx font pre=\sffamily,link to=event/text tag,symbol=eg]{text tag} is the node \texttt{main connector jikji}.
The \conceptname[link,link to=event/text tag,idx pre=event,idx font pre=\sffamily,no idx]{text tag} itself is the node \texttt{tag jikji}.
...
Leaping ahead, we now want to add Donald Knuth\chronosdocidx[type=tag,font=\sffamily,idx post=Donald Knuth\string\idxeg]{life}.
\begin{chronoscode}
  \chronoslife{%
    name=donald knuth,
    birth={1938-01-10},
...
\Pkg{chronos} assigns today's date internally for placement purposes, but will not typeset it when constructing the \conceptname[link,link to=life/text tag,idx pre=life,idx font pre=\sffamily]{text tag}\footnote{\pkg{chronos} is not the most optimistic of packages.}
...
\end{chronoscode}
If we wanted to override the formatting of the name rather than the dates, we could use\chronosdocidx[type=tag,font=\sffamily,idx post=WoOdBlOcK pRiNtInG\string\idxeg]{period}
...
\loadlist[Packages]{%
  ...
  {\pkgname[no link]{chronos-lib-colschemes} (part of \pkgname{chronos})},
  {\pkgname[no link]{chronos-lib-styles} (part of \pkgname{chronos})},
  {\pkgname[no link]{etoolbox}},
  {\pkgname[no link]{expl3} (if required)},
  {\pkgname[no link]{fontenc} (if using pdf\TeX{}; with option \option{T1})},
  {\pkgname[no link]{fp}},
  {\pkgname[no link]{pgfcalendar}},
  {\pkgname[no link]{svn-prov}},
  {\pkgname[no link]{tikz}},
  {\pkgname[no link]{xcolor} (with options \option{rgb,x11names,svgnames,dvipsnames})},
...
\Cref{subsec:mathau} provides an overview of the \conceptname[idx as=additional,idx pre=element,type=concept,font=\normalfont,idx font pre=\sffamily]{additional elements} which may be added in the \conceptname[type=concept,no idx,link]{\meta{timeline additions specification}}.
...
\begin{itemize}
  ...
  By default, borders are added when marks are placed on the \conceptname[idx post=borders,idx font post=\sffamily]{timeline} itself, which necessitates a taller \conceptname[idx post={effect of \string\textconcept{borders} on dimensions}]{timeline}.
  \item \Conceptname[target,idx post=location]{era labels} are (potentially) placed at each end of the line, depending on the time period covered.
...
The \conceptname[link,link pre=main/,idx pre=main,idx font pre=\sffamily]{frame} is constructed even later, but drawn behind most other \elementslabelname{} \keyname[type=style]{on chronos background layer}.
...
\paragraph{Timeline-Connectable Elements}\label{par:anatomi-add-timeline-connectable}\chronosdocidx[type=concept,idx pre=element,target pre=element/,target,font=\sffamily,idx font pre=\sffamily]{timeline-connectable}\chronosdocidx[type=element,target,font=\sffamily]{life,event,period}
% BEGIN par:anatomi-add-timeline-connectable
\Elementslabelname{} belonging to the first three \conceptname[type=concept,no idx,link,link to=tag]{tags} (\conceptname[type=tag,no idx,link,conj={,}]{life,event,period}) are (potentially) connected to the \conceptname[type=concept,idx post={elements, \string\textrm{additional}, timeline-connectable},link,idx font post=\sffamily]{timeline} and are placed according to date of occurrence.
...
\begin{itemize}
  ...
  A \conceptname[link,idx post=representation of time in \string\textconcept{life}/\string\textconcept{period}]{line} is (potentially) drawn and/or filled on, above or below the \conceptname[idx post=lines,idx font post=\sffamily]{timeline}, by default in the element's associated colour.
  ...
  A \conceptname[idx pre=event,idx font pre=\sffamily]{line} is (potentially) drawn on the \conceptname[idx post=connections,idx font post=\sffamily]{timeline}, by default in the element's associated colour.
\end{itemize}
...
\paragraph{Connectable Elements}\label{par:anatomi-add-connectable}\chronosdocidx[type=concept,idx pre=element,target pre=element/,target,font=\sffamily,idx font pre=\sffamily]{connectable}\chronosdocidx[type=element,target]{theory}
...
\begin{itemize}
  ...
  When the first three are connected to the \conceptname[type=concept,idx post=connectors,idx font post=\sffamily,link]{timeline}, one such connector is created by default\footnote{Connectors may be customised to ‘disappear’, but even invisible connectors can be used in connections.}.
  ...
  Unlike \conceptname[no idx,type=concept,link,link pre=element/]{timeline-connectable} \elementslabelname{} (\cref{par:anatomi-add-timeline-connectable}), they cannot be connected to the \conceptname[type=concept,idx post={elements, \string\textrm{additional}, non-timeline-connectable},link,idx font post=\sffamily]{timeline} and may be freely placed; unlike \conceptname[no idx,type=concept,link,link pre=element/]{non-connectable} \elementslabelname{} (\cref{par:anatomi-add-non-connectable}), they may be connected to each other and/or \conceptname[no idx,type=concept,link,link pre=element/]{timeline-connectable} \elementslabelname{}.
\end{itemize}
% END par:anatomi-add-connectable

\paragraph{Non-Connectable Elements}\label{par:anatomi-add-non-connectable}\chronosdocidx[type=concept,idx pre=element,idx font pre=\sffamily,target pre=element/,target,font=\sffamily]{non-connectable}\chronosdocidx[type=element,target,font=\sffamily]{info,theory circle}\chronosdocidx[type=element,target,idx pre=main,idx font pre=\sffamily,font=\sffamily,target pre=main/]{main title,frame}\chronosdochypertargets{element}{}{frame}\chronosdocidx[type=element,target,font=\sffamily,idx pre=copyleft,target pre=copyleft/,idx font pre=\sffamily]{copyleft,copyright}
\chronosdocidx[type=element,target,font=\sffamily,idx pre=copyright,idx font pre=\sffamily,target pre=copyright/]{copyleft,copyright}%
% BEGIN par:anatomi-add-non-connectable

\Elementslabelname{} belonging to the remaining \conceptname[type=concept,no idx,link,link to=tag]{tags} (\conceptname[type=tag,no idx,link]{info,theory circle,main,copyleft,copyright}) are \conceptname[link,type=concept,link pre=element/,idx font pre=\sffamily,no idx]{non-connectable} and, with the exception of \conceptname[link,no idx,link to=main/frame]{frame} may be located according to user preference.

\begin{itemize}
  \item Like \conceptname[no idx,type=concept,link,link pre=element/]{connectable}-but-not-\conceptname[no idx,type=concept,link,link pre=element/]{timeline-connectable} \elementslabelname{}, \conceptname[no idx,type=concept,link,link pre=element/]{non-connectable} \elementslabelname{} are not connected to the \conceptname[type=concept,idx post={elements, \string\textrm{additional}, non-timeline-connectable},idx font post=\sffamily,link]{timeline} and may involve no date information at all, but unlike \chronosdochypertargets{element}{connectable}{theory}\conceptname[type=tag,idx as=theory,idx post=cf.~\string\textconcept{non-connectable elements}]{theories}\chronosdocidx[type=concept,font=\sffamily,idx post={elements, \string\textrm{additional}, connectable},link,idx font post=\sffamily]{timeline} they do not feature \conceptname[idx as=text tag connector,idx post=not feature of \string\textconcept{non-connectable elements}]{connectors} so may not easily be connected to other \elementslabelname{}.
...
\begin{itemize}
  \item \Conceptname[idx as=\meta{tag}/connector,target,target as=\meta{tag}/connector]{connectors} are small \elementslabelname{} drawn on the boundaries of \conceptname[idx as=text tag]{text tags} and the \conceptname[idx post=connectors,idx font post=\sffamily]{timeline} which can be used as connection points.
  ...
  \item \Conceptname[idx as=\meta{tag}/line,target,target as=\meta{tag}/line]{lines} are marked on the \conceptname[idx post=lines,idx font post=\sffamily]{timeline} to indicate the date and/or duration of dated \elementslabelname{}.
\end{itemize}
...
Examples of different \conceptname[type=concept,idx as=tag,idx post=coordinate names]{tags}\chronosdocidx[type=concept,idx post=node names,font=\sffamily]{tag} have been added with labels to illustrate how \pkg{chronos} names their coordinates and nodes.
...
By default, it is designed to produce a picture occupying an entire A4 page and has a wide right-hand margin for additional \elementslabelname{}, in addition to ten \conceptname[idx post=cronoleg\string\idxeg,idx font post=\ttfamily]{levels} above and below the \conceptname[no idx,link,link to=timeline]{timeline}.
...
\begin{table}
  \begin{adjustwidth}{-\tewadjust}{0pt}
    ...
    \begin{tabularx}\linewidth{>{\ttfamily}l>{\ttfamily}X>{\ttfamily}lXlcc}
      \toprule
      ...
      \keyname[type=chronosstyle,idx post=features (summary)]{lavender menace} & on line & 3:3 & 1500--1900\,\ce{} & \keyname[type=colscheme,idx post=use by \string\texttt{lavender menace}]{lavender}+\texttt{Silver} & \fycheck{} & \fycross{}  \\
...
\DescribeChronosstyle{off line colour}\chronosdocidx[symbol=eg,idx pre=off line,type=chronosstyle]{off line colour}A straightforward style utilising scientific dates.
...
\DescribeChronosstyle{off line colour alt}\chronosdocidx[symbol=eg,idx pre=off line,type=chronosstyle]{off line colour alt}A variant of \keyname[link,idx=false,type=chronosstyle]{off line colour} which uses a different \colschemelabelname{}.
Intended for \conceptname[idx as=timeline,no idx,link,link to=timeline]{timelines} which add \elementslabelname{} above and/or below.
See \cref{tab:chronosstyles,subfig:off-line-col-alt}.

\DescribeChronosstyle{off line simple}\chronosdocidx[symbol=eg,idx pre=off line,type=chronosstyle]{off line simple}A less colourful variant of \keyname[link,idx=false,type=chronosstyle]{off line colour} utilising only two colours\texseans[%
...
\DescribeChronosstyle{simple arrow}\chronosdocidx[symbol=eg,idx pre=off line,type=chronosstyle]{simple arrow}A monochrome appearance with a plain 200mm arrow \conceptname[no idx,link]{timeline} and \conceptname[idx post=in \string\textkey{simple arrow}\string\idxeg]{years,marks} above\texseans{342699}.
Intended for \conceptname[idx as=timeline,no idx,link,link to=timeline]{timelines} which add \elementslabelname{} below.
...
\subsection{Chronos Colour Schemes}\label{subsec:chronos-schemes}\chronosdocidx[type=concept,target,font=\sffamily,idx post=using]{colour scheme}\vspace{-\baselineskip}
...
\dimkeyslabelname{} come in four variants:
...
When \dimkeyslabelname{} end in prime\chronosdocindex{' (prime)\actualchar\texttt{\textquotesingle} (prime)} or prime-plus\chronosdocindex{'+ (prime-plus)\actualchar\texttt{\textquotesingle+} (prime-plus)}, \meta{dimension}s  must be given as \TeX{} dimensions complete with units and may not require calculation.
...
In the case of the \conceptname[idx post=era labels,idx font post=\sffamily]{timeline}, era labels may instead be added at each end to avoid the clutter of including \bceyearlabel{} or \ceyearlabel{} with every year.
...
This is the format used to format ‘major’ years on the \conceptname[idx post={years\string\textrm{,} major\string\textrm{, format}},idx font post=\sffamily]{timeline}.
...
Should era labels be included at the end(s) of the \conceptname[idx post=era labels,idx font post=\sffamily]{timeline}?
...
Generally, this works well, but an issue occurs if your \conceptname[idx post=spanning eras,link,idx font post=\sffamily]{timeline} spans the two eras (\bceyearlabel{} and \ceyearlabel{}).
...
%     \showthe\linewidth
    \subcaptionbox{\pkg{pgfcalendar} default.\label{subfig:sero}}{%
      \begin{chronos}
        ...
        \chronosperiod {dates={0-01-01}:{0-12-31},name=non-existent year,colour=WildStrawberry}
      \end{chronos}%
    }%
...
First, what should be marked on the \conceptname[idx post=if no year zero,link,idx font post=\sffamily]{timeline} at the ‘era switch’?
...
Second, \pkg{chronos} provides a special option for configuring what is marked on the \conceptname[idx post=era switch,link,idx font post=\sffamily,no idx]{timeline} at the switch of eras.
...
If false, the year (e.g.~‘1’) is used; if true, a mark is used instead (illustrated in \cref{subfig:heb-sero}, though the format will depend on how the \conceptname[idx post=era switch,link,idx font post=\sffamily]{timeline} is configured).
\Default{true}
...
By default, it is used to determine the colours for the \conceptname[idx post=colours,link,idx font post=\sffamily]{timeline} itself and is the basis for the colours used in some \tagslabelname.
It is also used in some standard \conceptname[type=colour,idx as=chronos main background colour,font=\ttfamily,idx post=use in \string\textsf{chronos styles}]{chronos styles}.
\Default{white}
\begin{chronoscode}
  \begin{chronos}
    [
...
Whether it is used and, if so, how, depends on other settings.
By default, it is used to determine the colours for the \conceptname[idx post=colours,link,idx font post=\sffamily,no idx]{timeline} itself and is the basis for the colours used in some \tagslabelname.
It is also used as the \keyname[link,idx=false,type=colkey,link prefix=\meta{tag}/,idx as=\meta{tag}/default colour]{default colour} for \conceptname[idx as={connection,line,text tag},idx post=default use of \string\textkey{chronos main colour} in]{connections,lines,text tags} and in some standard \conceptname[type=colour,idx as=chronos main colour,font=\ttfamily,idx post=use in \string\textsf{chronos styles}]{chronos styles}.
\Default{black}
\begin{chronoscode}
  \begin{chronos}
    [
...
Which  \conceptname[type=layer,no link,idx as={connections on,lines on,timeline/timeline on,timeline/border on},font=\ttfamily]{layer} each type of element should be placed on.
...
\DescribeDimkey{timeline/timeline height,timeline/height,timeline/timeline height+,timeline/height+,timeline/timeline height\textquotesingle,timeline/height\textquotesingle,timeline/timeline height\textquotesingle +,timeline/height\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{timeline/timeline border height,timeline/timeline border height+,timeline/timeline border height\textquotesingle,timeline/timeline border height\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{timeline/timeline width,timeline/timeline width+,timeline/timeline width\textquotesingle,timeline/timeline width\textquotesingle +,timeline/width,timeline/width+,timeline/width\textquotesingle,timeline/width\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{timeline/timeline margin,timeline/timeline margin+,timeline/timeline margin\textquotesingle,timeline/timeline margin\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{timeline/timeline era margin,timeline/timeline era margin+,timeline/timeline era margin\textquotesingle,timeline/timeline era margin\textquotesingle +}\val{dimension}\AfterLastParam
The horizontal space to allow between the first/last point on the \conceptname[idx post=era margins,idx font post=\sffamily]{timeline} and the era labels.
\Default{15pt}
For example,
\begin{chronoscode}
timeline={
...
\DescribeDimkey{headings border,headings border+,headings border\textquotesingle,headings border\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{headings drop,headings drop+,headings drop\textquotesingle,headings drop\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{subheadings drops,subheadings drops+,subheadings drops\textquotesingle,subheadings drops\textquotesingle +}\cvalsmarg{dimension 1,dimension 2}\AfterLastParam
...
\DescribeDimkey{headings drops\textquotesingle,headings\textquotesingle +}\cvalsmarg{dimension 1,dimension 2,dimension 3}\AfterLastParam
A convenience key equivalent to setting \keyname[type=dimkey,no idx]{headings drop\textquotesingle} to \meta{dimension 1} and \keyname[type=dimkey,no idx]{subheadings drops\textquotesingle} to \meta{dimension 2} and \meta{dimension 3}.
...
\DescribeDimkey{outer border,outer border+,outer border\textquotesingle,outer border\textquotesingle +}\val{dimension}\AfterLastParam
...
\DescribeDimkey{borders\textquotesingle,borders\textquotesingle +}\cvalsmarg{dimension,dimension,dimension,dimension,dimension,dimension}\AfterLastParam
...
\DescribeDimkey{top border,top border+,top border\textquotesingle,top border\textquotesingle +,right border,right border+,right border\textquotesingle,right border\textquotesingle +,bottom border,bottom border+,bottom border\textquotesingle,bottom border\textquotesingle +,left border,left border+,left border\textquotesingle,left border\textquotesingle +}\val{dimension}\AfterLastParam
...
\Default{0pt}
\emph{Most people should let the frame use the bounding box\chronosdocidx[idx pre=tikzpicture,type=env,idx font pre=\ttfamily]{bounding box}, which is the default, and leave these dimensions alone.}
...
\Cref{fig:timeline-dims} shows a \conceptname[idx post={marks \string\textrm{and} years\string\textrm{, invisible}},idx font post=\sffamily]{timeline} in which this has been done by setting the foreground and background colours equal.
...
\chronosdocidx[type=choice,idx pre=timeline/timeline years,target prefix=timeline/timeline years/,target,idx font pre=\ttfamily]{on line,off line,above,below,none}%
\AfterLastParam
...
Instead, put the years of subsequently added \conceptname[type=element,idx as=event,idx post=event years on line,idx font post=\ttfamily]{events} onto the line.
...
How often to label \conceptname[idx post=frequency of labelling]{major years} on the \conceptname[idx post=major years,idx font post=\sffamily]{timeline} if showing them.
...
How often to label \conceptname[idx post=frequency of labelling]{minor years} on the \conceptname[idx post=minor years,idx font post=\sffamily]{timeline} if displaying them.
...
How often to label \conceptsname[idx post=frequency of labelling]{year} on the \conceptname[idx post=years,idx font post=\sffamily]{timeline}, if you want them all to be formatted in the same way.
...
If for some reason you do \emph{not} want \conceptsname[idx post=non-modulo configuration]{year} on the \conceptname[idx post={years\string\normalfont, not modulo},idx font post=\sffamily]{timeline} to be determined modulo \keyname[link,link prefix=timeline/,idx=false]{step major year,step minor year}, you may tell \pkg{chronos} where to begin stepping from.
...
\emph{Adds} \meta{key-value list} to the common \stylelabelname{} used when putting \conceptname[idx post={style in common with}]{major years,minor years} onto the \conceptname[idx post={years\string\textnormal{, style}},idx font post=\sffamily]{timeline}.
...
The \TikZ{} \keyname[no link,idx post=for \string\textconcept{years}]{anchor} to use when creating the \texttt{nodes} for \conceptname[idx post=set \string\TikZ{} anchor]{years} on or off the \conceptname[idx post={years\string\textrm{,} \string\texttt{anchor}},idx font post=\sffamily]{timeline}.
...
Whether to draw vertical marks on or off the \conceptname[idx post=marks,idx font post=\sffamily]{timeline} at \conceptname[idx post=setting \string\textconcept{marks}]{major years} using the \stylelabelname{} set with \keyname[link,link prefix=timeline/,idx=false]{timeline mark}.
\Default{true}
\Initial{true}

\DescribeBoolkey{timeline/timeline minor marks}\vals{true,false}\AfterLastParam
Whether to draw vertical marks on or off the \conceptname[idx post=minor marks,idx font post=\sffamily]{timeline} at \conceptname[idx post=setting \string\textconcept{minor marks}]{minor years} using the \stylelabelname{} set with \keyname[link,link prefix=timeline/,idx=false]{timeline minor mark}.
\Default{true}
\Initial{true}

\DescribeBoolkey{timeline/timeline show years}\vals{true,false}\AfterLastParam
Whether to represent years on or off the \conceptname[idx post=years,idx font post=\sffamily]{timeline} at all.
...
Whether to draw \conceptname{bare marks} on or off the \conceptname[idx post=bare marks,idx font post=\sffamily]{timeline} in between \conceptsname[no idx]{year}\footnote{If your \conceptname[no idx]{timeline} is very short and 12 \keyname[link,link prefix=timeline/,idx=false]{step divisions} are set, \pkg{chronos} will actually mark months.
...
\emph{Adds} to the \stylelabelname{} used for the vertical lines drawn when \pkg{chronos} labels a \conceptname[idx post=marks,idx font post=\ConceptFont]{major year} on or off the \conceptname[idx post={marks, adding to style},idx font post=\sffamily]{timeline} and \keyname[link,link prefix=timeline/,idx=false,type=boolkey]{timeline marks} is true.
These correspond to the \conceptname{major steps} at which \pkg{chronos} puts \conceptname[idx post=marks \string\textnormal{for }major,idx font post=\sffamily]{years}.
...
The \stylelabelname{} used to mark \conceptsname[idx post=event year on line,idx font post=\ttfamily]{year} on the \conceptname[idx post=event year on line,idx font post=\KeyFont]{timeline} if \keyname[idx=false,link]{event years on line} is enabled.
...
The colour used for the background of the central part of the \conceptname[idx post=colours,idx font post=\sffamily]{timeline}.
...
The colour used for the foreground of the central part of the \conceptname[idx post=colours,idx font post=\sffamily,no idx]{timeline}.
...
The \conceptname[link,idx post=style]{timeline}'s overall style can be customised using the following \keyslabelname{}, which should (and, by default, do) utilise \conceptname[idx pre=timeline,idx font pre=\sffamily]{colours} from the \conceptname[link,no idx,type=concept]{colour scheme} (see \cref{subsec:chronos-style-creation}).
Unless you are creating a \conceptname[type=concept,link,no idx]{chronos style}, it is best to \emph{add to} rather than \emph{replacing} the existing configuration.XXX
...
The style of the \conceptname[link,idx pre=timeline,idx font pre=\sffamily]{timeline line}.
...
timeline={%
  ...
  timeline line={Bar-Latex,chronos timeline foreground colour,double=chronos timeline background colour},
  ...
}
\end{chronoscode}
...
The style of the \conceptname[link,idx pre=timeline,idx font pre=\sffamily]{timeline border}.
...
This is true by default, but the associated style is empty so the frame is invisible.
\Default{true}
\Initial{true}
...
The idea is to tell \pkg{chronos} how many tiers (approximately) of \elementslabelname{} you will create above and below the \conceptname[idx post=levels,idx font post=\sffamily]{timeline}.
For each of these levels, \pkg{chronos} creates a standardised node or \keyname[type=choice,idx as=placeholders,link to=placeholders,idx post=levels,idx font post=\sffamily]{placeholder} based on the settings used for \elementslabelname{} of type \conceptname[type=tag,idx post=as basis for \string\textconcept{levels}]{life} when the \conceptname[no idx]{timeline} is constructed.
...
\meta{number above} and \meta{number below} should be non-negative integers specifying how many levels to create above and below the \conceptname[idx post={levels\string\textnormal{, creating}},idx font post=\sffamily]{timeline} respectively.
...
Although they are not intended to be visible in the \conceptname[idx post={levels\string\textnormal{, rendering visible}},idx font post=\sffamily]{timeline}, placeholder nodes may be rendered visible for debugging or development purposes.
...
Create a \conceptname[idx post=century subheadings,idx font post=\ttfamily,link]{subheading} aligned with \texttt{chronos lower subheadings} for each of the centuries specified in \val{number list}, using \meta{text} as the superscript for each.
...
\subsubsection{Colour Rotation}\label{subsubsec:troi-lliwiau}\chronosdocidx[type=concept,target,font=\sffamily]{colour rotation}\vspace{-\baselineskip}
% BEGIN subsubsec:troi-lliwiau

More complex configuration can be achieved using lists of colours from which \pkg{chronos} selects when adding elements to the \conceptname[idx as=additional,idx post=and \string\textconcept{colour rotation}]{timeline}.
...
\subsubsection{Colour Lists}\label{subsubsec:collist}\chronosdocidx[type=concept,target,font=\sffamily]{colour list}
...
%     \begin{tabularx}\chronosdoctemplgth{>{\ttfamily}Xccccccc}
    ...
    \chronosdocidx[type=tag,font=\sffamily,idx post={options (summary)}]{life,event,period,theory,info,theory circle,main,copyleft,copyright}
...
These are described in \cref{tab:life-period} for a typical case, but note that additional \conceptname[idx pre=life,idx font pre=\sffamily]{connectors} require \keyname[link prefix=life/,idx as=life/connectors]{connectors} to be set, the \conceptname[idx pre=life,idx font pre=\sffamily]{connection} is drawn only if \keyname[link prefix=life/,type=boolkey,idx as=life/connect]{connect} is \texttt{true} and some components may be rendered invisibly.
\begin{table}
  \begin{adjustwidth}{-\tewadjust}{0pt}
    \centering
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{life,period}%
    \chronosdocidx[type=tag,idx pre=life,idx font pre=\sffamily,font=\sffamily]{chronos connector,main connector,text tag connector,connectors,line,text tag,connection}%
    \chronosdocidx[type=tag,idx pre=period,idx font pre=\sffamily,font=\sffamily]{chronos connector,main connector,text tag connector,connectors,line,text tag,connection}%
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      ...
      \conceptname[idx post=as component of \string\textconcept{life}]{line}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{line} & \fycross{} & Line or rectangle on or near \conceptname[no idx]{timeline} or \conceptname[idx post=as location of \string\textconcept{line}]{timeline border} from \meta{birth} to \meta{death}  (\conceptname[no idx]{life}) or \meta{start} to \meta{end} (\conceptname[no idx]{period}). & \cs{path} \\
      \conceptname[idx post=as component of \string\textconcept{life}]{chronos connector}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{chronos connector} & \keyname[type=node,no link,idx post=as component of \string\textconcept{life} and \string\textconcept{period}]{chronos connector \meta{name}} & Connection point midway along \conceptname[no idx]{line}. & node \\
      \conceptname[idx post=as component of \string\textconcept{life}]{text tag}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{text tag} & \keyname[type=node,no link,no link,idx post=as component of \string\textconcept{life} and \string\textconcept{period}]{tag \meta{name}} & Main box representing \elementlabelname{}. By default, contains dates above capitalised \meta{name} (\conceptname[no idx]{life}) or capitalised \meta{name} above dates (\conceptname[no idx]{period}). & node \\
      \conceptname[idx post=as component of \string\textconcept{life}]{main connector}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{main connector} & \keyname[type=node,no link,idx post=as component of \string\textconcept{life} and \string\textconcept{period}]{main connector \meta{name}} & Connection point at \TikZ{} anchor of \conceptname[no idx]{text tag}. & node \\
      \conceptname[idx as=connection,idx post=as component of \string\textconcept{life}]{connection}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{connection} & \fycross{} & Line between the \conceptname[no idx]{chronos connector,main connector}. & \cs{draw} \\
      \conceptname[idx as=text tag connector,idx post=as component of \string\textconcept{life}]{connectors}\chronosdocidx[type=element,idx post=as component of \string\textconcept{period},font=\sffamily]{text tag connector} & \keyname[type=node,no link,idx post=as component of \string\textconcept{life} and \string\textconcept{period}]{connector \meta{name}\textrm{\textit{n}}} & Secondary connection point(s) at \TikZ{} anchor(s) of \conceptname[no idx]{text tag}, named in order with $n=1,2,\ldots$. & node \\ \bottomrule
    \end{tabularx}
  \end{adjustwidth}
\end{table}
...
If more complex paths are required, \texttt{connect=false} may be used and the \conceptname[no idx,link]{text tag} connected to the \conceptname[idx post={connections, \string\textrm{complex}},idx font post=\sffamily]{timeline} manually.
A \conceptname{chronos connector}, \keyname[type=node,no link,symbol=eg,no idx]{chronos connector leslie lamport}, would then be created on the \conceptname[idx post=connectors,idx font post=\sffamily]{timeline}, as would the \conceptname[idx as=connector,link,link to=connector,no idx]{connectors} on the \conceptname[no idx,link]{text tag}, but the \conceptname[no idx,link]{connection} itself would be omitted.
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{event}
    \chronosdocidx[type=tag,idx pre=event,idx font pre=\sffamily,font=\sffamily]{chronos connector,main connector,text tag connector,connectors,line,text tag,connection}%
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
Creating the \elementlabelname{} involves constructing, naming and connecting several components.
These are described in \cref{tab:life-period} for a typical case, but note that additional \conceptname[idx as=text tag connector,idx pre=period,idx font pre=\sffamily]{connectors} require \keyname[link prefix=period/,idx as=period/connectors]{connectors} to be set, the \conceptname{connection} is drawn only if \keyname[link prefix=period/,type=boolkey,idx as=period/connect]{connect} is \texttt{true} and some components may be rendered invisibly.
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{theory}
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{theory circle}
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{info}
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post={components}]{main}
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
%     \chronosdocidx[type=tag,font=\sffamily,idx post=components]{copyleft,copyright}
    \begin{tabularx}{\textwidth+\tewadjust}{l>{\ttfamily}lX>{\ttfamily}l}
      \toprule
      \sffamily \Conceptname[type=concept,no idx]{element} & \sffamily Name & \sffamily Description & \TikZ{} Type \\\midrule
...
\subsection{Additional Elements: Local Configuration}\label{subsec:add-elements-lconfig}\chronosdocidx[type=tag,font=\sffamily,idx post={configuration, local}]{life,event,period,theory,info,theory circle,main,copyleft,copyright}\vspace{-\baselineskip}
% BEGIN subsec:add-elements-lconfig

The \keyslabelname{} are designed for use when creating specific \elementslabelname{}.
...
Except for \keyname[type=macro,idx post=name\string\textnormal{ optional for},idx font post=\ttfamily]{\cs{chronosmaintitle},\cs{chronoscopyleft},\cs{chronoscopyright}}, this key is required.
...
This key is mandatory for \conceptname[type=tag,idx post=at \string\textrm{mandatory},idx font post=\ttfamily]{theory circle,info,main,copyleft,copyright}.

For \conceptname[type=tag,idx post=at \string\textrm{optional},idx font post=\ttfamily]{life,event,period,theory}, the key is optional.
...
Since \conceptname[type=tag,idx post=text tags \string\textrm{dateless},idx font post=\sffamily]{theory} text tags do not have dates, they are placed at the \keyname[type=coord,no link]{(chronos origin)} and a warning is issued.
...
The \pgf/\TikZ{} anchor to use for the \conceptname[idx as=main connector,idx post=anchor,idx font post=\ttfamily]{element}'s \conceptname[no idx]{main connector}.
...
Dates  of a \conceptname[type=tag,conj=or,link,idx post=dates,idx font post=\ttfamily]{life,period}, specified as explained in \cref{subsec:dyddiadau}.
...
The date  of an \conceptname[type=tag,link,idx post=date,idx font post=\ttfamily]{event}, specified as explained in \cref{subsec:dyddiadau}.
...
\DescribeDimkey{sizes,sizes+,sizes\textquotesingle,sizes\textquotesingle +}\cvals{outer circle dimension,inner circle dimension} \fortags[names={sizes,sizes+,sizes\textquotesingle,sizes\textquotesingle +},type=dimkey]{theory circle}\AfterLastParam
...
\chronosdocidx[type=tag,font=\sffamily,idx post={configuration,local/global}]{life,event,period,theory,info,theory circle,main,copyleft,copyright}\vspace{-\baselineskip}
...
The ‘main’ \conceptname[idx post={main\string\textnormal{, identifying}},idx font post=\sffamily]{connector} is the one which connects (or would connect) the \conceptname[idx post={main connector\string\textnormal{, identifying}},idx font post=\sffamily]{text tag} to the \conceptname[no idx]{timeline}.
...
\chronosdocidx[type=tag,font=\sffamily,idx post={configuration,global}]{life,event,period,theory,info,theory circle,main,copyleft,copyright}\vspace{-\baselineskip}
...
Configuration of the base ring for \elementslabelname{} of \conceptname[type=concept,idx post=theory circle,idx font post=\sffamily]{tag} type \conceptname[type=tag,idx post=configuring base ring]{theory circle}.
...
\DescribeDimkey{text tag yshift,text tag yshift+,text tag yshift\textquotesingle,text tag yshift\textquotesingle+}\val{dimension} \fortags{life,event,period,theory}\AfterLastParam
...
\chronosdocidx[type=tag,font=\sffamily,idx post={colours,using}]{life,event,period,theory,info}%
\chronosdocidx[type=tag,font=\sffamily,idx post={styles,using}]{life,event,period,theory,info}\vspace{-\baselineskip}
...
In this case, \keyname[type=element,idx pre=text tag,idx font pre=\sffamily,symbol=eg,no link]{tag johannes gutenberg} is being \conceptname[idx as=connection,idx post=between \string\textkey{johannes gutenberg} and other \string\textconcept{elements}\string\idxeg]{connected} to \keyname[type=element,idx pre=text tag,idx font pre=\sffamily,symbol=eg,no link]{tag printing press}, \keyname[type=element,idx pre=text tag,idx font pre=\sffamily,symbol=eg,no link]{tag movable type} and \keyname[type=element,idx pre=text tag,idx font pre=\sffamily,symbol=eg,no link]{tag gutenberg bible}.
...
\Pkgname[idx post=memoize\string\textnormal{-ready},idx font post=\PkgFont]{chronos} supports automemoization out-of-the-box\footnote{%
  ...
  Even I managed to do achieve this without major incident.}: to enable automatic memoization of \keyname[type=env,idx post=enable automemoization]{chronos} \envslabelname{}, simply load \pkg{memoize} early in your preamble.
...
As explained in \cref{subsec:chronos-schemes}, in addition to the default colours, \pkg{chronos} currently provides \keyname[type=colscheme,idx post=as instance of custom\string\idxeg]{blues,contninety,cronoleg,lavender,modern,offlinebasic,offlinealt,sobriety,xcolseries} (\cref{tab:colschemes}).
...
\begin{table}
  \begin{adjustwidth}{-\tewadjust}{0pt}
    ...
    \begin{tabularx}\linewidth{>{\ttfamily}ll>{\ttfamily\raggedright\arraybackslash}X}
      ...
      \hypertarget{colschemekey:foreground}{foreground} & \meta{colour name} & black \\ %  {\__chronos_color_set_from_existing:nn {chronos@prifliw}{#1}} %  chronos main colour  &
      \hypertarget{colschemekey:background}{background} & \meta{colour name} & white \\ %  {\__chronos_color_set_from_existing:nn {chronos@prifliw@cefndir}{#1}} %  chronos main background colour  &
      \hypertarget{colschemekey:timeline-foreground}{timeline foreground} & \meta{colour name} & Cerulean  \\ %  {\__chronos_color_set_from_existing:nn {chronos@lliw@llinell}{#1}} %  chronos timeline foreground colour  &
      \hypertarget{colschemekey:timeline-background}{timeline background} & \meta{colour name} &  DodgerBlue4!50!black  \\ %  {\__chronos_color_set_from_existing:nn {chronos@lliw@cefndir@llinell}{#1}} %  chronos timeline background colour    &
      \hypertarget{colschemekey:timeline-border-outer}{timeline border outer}  & \meta{colour name} & white  \\ %  {\__chronos_color_set_from_existing:nn {chronos@borderouter}{#1}} %  timeline border outer colour  &
      \hypertarget{colschemekey:timeline-border-inner}{timeline border inner}  & \meta{colour name} & Cerulean  \\ %  {\__chronos_color_set_from_existing:nn {chronos@borderinner}{#1}} %  timeline border inner colour  &
      \hypertarget{colschemekey:timeline-border-middle}{timeline border middle}  & \meta{colour name} &  DodgerBlue4!50!black \\ %  {\__chronos_color_set_from_existing:nn {chronos@bordermiddle}{#1}} %   timeline border middle colour &
      \cmidrule(lr){1-3}
      \hypertarget{colschemekey:life/default}{life/default} & \meta{colour name} & DodgerBlue4  \\ %  {\__chronos_color_set_from_existing:nn {chronos@byw@lliw@rhagosodedig}{#1}} %  life/default colour  &
      \hypertarget{colschemekey:event/default}{event/default} & \meta{colour name} & DodgerBlue4  \\ %  {\__chronos_color_set_from_existing:nn {chronos@digwyddiad@lliw@rhagosodedig}{#1}} %  event/default colour  &
      \hypertarget{colschemekey:period/default}{period/default} & \meta{colour name} & DodgerBlue4  \\ %  {\__chronos_color_set_from_existing:nn {chronos@parhad@lliw@rhagosodedig}{#1}} %  period/default colour  &
      \hypertarget{colschemekey:theory/default}{theory/default} & \meta{colour name} & DodgerBlue4  \\ %  {\__chronos_color_set_from_existing:nn {chronos@theori@lliw@rhagosodedig}{#1}} %  theory/default colour  &
      \hypertarget{colschemekey:info/default}{info/default} & \meta{colour name} & DodgerBlue4  \\ %  {\__chronos_color_set_from_existing:nn {chronos@gwybodaeth@lliw@rhagosodedig}{#1}} %  info/default colour  &
      \cmidrule(lr){1-3}
      \hypertarget{colschemekey:default-above}{default above} & \meta{list of colour names} &  Red, Orange, Yellow, Green, Blue, MidnightBlue, Violet \\ %  \g__chronos_lliwiau_uchod_clist %  colours above  &
      \hypertarget{colschemekey:default-below}{default below} & \meta{list of colour names} & Cerulean!50!DodgerBlue4, Cerulean!50!DodgerBlue3, Cerulean!50!DodgerBlue2, Cerulean!50!DodgerBlue1, Cerulean \\ %  \g__chronos_lliwiau_isod_clist %  colours below  &
      \hypertarget{colschemekey:life/above}{life/above} & \meta{list of colour names} & DeepPink2, DarkOrange1, Firebrick1, Purple0, WildStrawberry, OrangeRed1, DarkGoldenrod1, DarkOrchid3 \\ %  \g__chronos_lliwiau_byw_uchod_clist %   life/colours above  &
      \hypertarget{colschemekey:life/below}{life/below} & \meta{list of colour names} & DodgerBlue3, Green3, Blue3, SpringGreen4, DeepSkyBlue2, ForestGreen, Periwinkle, SeaGreen3 \\ %  \g__chronos_lliwiau_byw_isod_clist %   life/colours below  &
      \hypertarget{colschemekey:event/above}{event/above} & \meta{list of colour names} & Thistle4, Thistle4!.5!Thistle3, Thistle3, Thistle3!.5!Thistle2, Thistle2 \\ %  \g__chronos_lliwiau_digwyddiad_uchod_clist %   event/colours above  &
      \hypertarget{colschemekey:event/below}{event/below} & \meta{list of colour names} & Seashell4, Seashell4!.5!Seashell3, Seashell3, Seashell3!.5!Seashell2, Seashell2 \\ %  \g__chronos_lliwiau_digwyddiad_isod_clist %   event/colours below  &
      \hypertarget{colschemekey:period/above}{period/above} & \meta{list of colour names} & MistyRose4, MistyRose4!.5!MistyRose3, MistyRose3, MistyRose3!.5!MistyRose2, MistyRose2 \\ %  \g__chronos_lliwiau_parhad_uchod_clist %   period/colours above  &
      \hypertarget{colschemekey:period/below}{period/below} & \meta{list of colour names} & Ivory4, Ivory4!.5!Ivory3, Ivory3, Ivory3!.5!Ivory2, Ivory2 \\ %  \g__chronos_lliwiau_parhad_isod_clist %   period/colours below  &
...
\tikzset{%
  blues below/.style={%
    ...
    timeline={%
      ...
      timeline year={fill=none,rotate around={45:(chronos year \chronosyeari)},text=chronos timeline foreground colour},
...
\begin{enumerate}
  ...
  \item\label{item:weird-i} there is a weird looking \cs{chronosyeari} in the definition of \keyname[link prefix=timeline/,idx as=timeline/timeline year]{timeline year};
...
%     \chronosdochypertargets{colour}{}{core,core derivative,core border}%
    \chronosdocidx[type=colour,font=\sffamily,target]{core,core derivative,core border,elemental}%
...
\cs{chronosyeari} refers to the current \conceptname{year}.
\texttt{(chronos year \cs{chronosyeari})} isn't actually the node, but the point representing the date on the \conceptname[idx post=chronos year \string\cs{chronosyeari},idx font post=\ttfamily]{timeline}, but the node starts there, so we can use it provided \keyname[link prefix=timeline/,idx as=timeline/timeline years anchor]{timeline years anchor} is set appropriately.
\begin{chronoscode}
  timeline year={rotate around={45:(chronos year \chronosyeari)}},
  timeline years anchor=south west,
\end{chronoscode}

% END subsubsec:rotate-years

\subsubsection{Hashes}\label{subsubsec:hashes}\chronosdocidx{\string\verb*\verbatimchar\string#\verbatimchar\string\space (hash)}\chronosdocidx[font=\normalfont]{hash (\string\verb*\verbatimchar\string#\verbatimchar )}XXX
...
\begin{enumerate}
  ...
  \Pkg{chronos} will keep assigning colours to \elementslabelname{} of \conceptname[type=concept,link,no idx]{tag} type \conceptname[type=tag,idx post=colour rotation,idx font post=\sffamily]{event}, but it will not assign ‘red’ except by happy chance.
...
\Example{}Here are the definitions of \keyname[type=keyhandler,no link,idx pre=.chronos key maker,idx font pre=\ttfamily,symbol=eg]{tag left,tag post,tag right} mentioned above.
...
If, and only if, \keyname[type=boolkey,idx as=timeline/timeline mark eras,idx post=effect of enabling explicitly \string\emph{vs.}~implicitly on \string\textkey{show nodes},link prefix=timeline/,link]{timeline mark eras} is explicitly enabled (as opposed to being enabled just because a \conceptname[no idx]{timeline} spans \bceyearlabel{} and \ceyearlabel{}), draws and labels the nodes containing the era labels on the \conceptname[idx post=show nodes,idx font post=\ttfamily]{timeline}.
\Default{true}
\Initial{false}
...
For example, if a user specifically requests \keyname[type=choice,link prefix=timeline/timeline years/,idx pre=timeline/timeline years,idx font pre=\ttfamily]{off line} \conceptsname[idx post=effects of configuration]{year} with a \keyname[type=dimkey,idx as=timeline/timeline height,link prefix=timeline/,link]{timeline height} of 50mm in white-on-blue, \pkg{chronos} won't override those settings.
...
\begin{itemize}
  ...
  This applies to cases where either \pkg{chronos} uses a command if it is available (e.g.~\keyname[type=macro,no link,idx pre=conditionally used,idx as=\cs{uistyle}\string\textrm{\string\idxeg}]{\cs{uistyle}}), but needs a fallback otherwise, or a public macro is made available as a convenience, if the user is not using the name already (e.g.~\keyname[type=macro,idx pre=conditionally defined,idx as=\cs{celabel}\string\textrm{\string\idxeg}]{\cs{celabel}}).
  These \macroslabelname{} are listed in \cref{tab:public,tab:provides}.
  \begin{table}
    \centering
    \caption{Public names for \pkgname[no link]{chronos} internal macros defined if undefined at the end of the preamble.}\label{tab:public}
    \begin{tabular}{ll}
      \toprule
      \sffamily Public name & \sffamily\keyname[type=pkg,no link,idx as=chronos,font=\sffamily,idx post=internal macro names]{\Pkg{chronos}} internal name  \\\midrule
  ...
  \begin{table}
    ...
    \begin{tabular}{ll}
      \toprule
      \sffamily Functionality used if defined & \sffamily\keyname[type=pkg,no link,font=\sffamily,idx as=chronos,idx post=fallback macro definitions]{\Pkg{chronos}} fallback definition  \\\midrule
      \keyname[type=macro,no link]{\cs{tlstyle}} &  \cs{let}\cs{tlstyle}\cs{upshape} \\
      \keyname[type=macro,no link]{\cs{uistyle}} &  \cs{let}\cs{uistyle}\cs{itshape} \\
  ...
  \begin{table}
    ...
    \begin{tabular}{>{\ttfamily}ll}
      \toprule
      \sffamily\pkgname[no link]{etoolbox} & \sffamily\keyname[type=pkg,no link,idx as=chronos,font=\sffamily,idx post=replacements for \string\textsf{etoolbox} macros]{\pkg{chronos}} \pkgname[no link]{expl3} wrapper \\\midrule
...
\begin{chronosmethod}
  \item\label{method:complete}\chronosdocidx[idx post=updating from \string\TeX{}\space SE code,font=\sffamily]{timeline} If you intend to develop work utilising code from \TeX{} SE answers further, I strongly recommend taking the time to switch to the new key-value interface and \enviro{chronos} \envlabelname{}.
  ...
  \item\label{method:no-chronos}\chronosdocidx[idx post=updating with retained \string\TeX{}\space SE code,font=\sffamily]{timeline} If you don't intend to develop existing \conceptname[idx as=timeline,idx post=completed using \string\TeX{}\space SE code]{timelines} further, I strongly recommend not loading \pkg{chronos}, renaming any existing file to avoid conflicts and doing an ultra-simple update so existing documents load the renamed file.
  ...
  \item\label{method:hinterlands} If \cref{method:complete,method:no-chronos} aren't options --- if, say, you want to use this package for a new \conceptname[no idx]{timeline} in a document with existing \conceptname[idx as=timeline,idx post=combining package and \string\TeX{}\space SE code in single document]{timelines} and you don't have time to update those, then one of the following pairs of definitions \emph{may} produce more-or-less the same output from existing or slightly modified code.
  ...
  \begin{chronosmethod}
    ...
    You will need to modify existing \conceptname[idx as=timeline,idx post=combining package and \string\TeX{}\space SE code in single document with minimal changes]{timelines} to use the \enviro{chronos} \envlabelname{} if they are currently in \enviro{tikzpicture} \envslabelname{}.
...
\end{verbatim}
    ...
    \item\label{method:rename}\chronosdocidx[font=\sffamily,idx post=combining package and \string\TeX{}\space SE code in single document with legacy names]{timeline} Alternatively, update all existing \envslabelname{} to use \enviro{chronos} as explained in \cref{method:cy}, if required.
