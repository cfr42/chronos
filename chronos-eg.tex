...
\svnidlong
{$HeadURL: file:///mnt/between/svn/tex/chronos/trunk/chronos-eg.tex $}
{$LastChangedBy: cfrees $}
{$LastChangedRevision: 10861 $}
{$LastChangedDate: 2025-02-25 17:05:56 +0000 (Maw, 25 Chw 2025) $}
% best not to mix \svnid and \svnidlong (or to always typeset time-zone with time? see documentation)
%    \svnid{$Id: chronos-eg.tex 10861 2025-02-25 17:05:56Z cfrees $}
...
% \makeatletter
% \DeclareRobustCommand{\PTSansNarrowstyle}{%
%   \not@math@alphabet\PTSansNarrowstyle\relax
%   \fontfamily{PTSansNarrow-TLF}\selectfont}
% \makeatother
% \DeclareTextFontCommand{\textPTSansNarrow}{\PTSansNarrowstyle}
\setlogokern{La}{-0.2em}
\setlogokern{aT}{-0.05em}

\begin{document}
% \begin{tikzpicture}
%   [chronos={% dimension too large (from a question about the same error in chronosys)
%     start year={-200000},
%     end year=2020,
% %     width=5000mm,
%   }]
%   \chronosevent{%
%     dyddiad={-100000}-12-01,
%     enw=something,
%   } 
% \end{tikzpicture}
% \newcount\mycount
% \foreach \i/\j/\k in {-1/12/30,-1/12/31,1/01/01,1/01/02} {\i--\j--\k : \pgfcalendardatetojulian{{\i}-\j-\k}{\mycount}\the\mycount{} ** }
% \pgfcalendardatetojulian{{-1}-12-31}{\mycount}\the\mycount:\pgfcalendardatetojulian{1-01-01}{\mycount}\the\mycount
%   \PTSansNarrowstyle
% \sffamily
% \iffalse
% ateb: https://tex.stackexchange.com/a/324106/
...
\begin{chronos}
  [
    blues below,
    timeline={%
      dates={2014-11-16}:{\year-\month-\day},
      width=\linewidth,
    }
  ]
  \chronosevent{%
    date={2014-11-16},
    name=Western Philosophy Timeline Finalised,
  }
  \chronosevent{
    date={2016-03-26},
    name=first,
%       name content={First answer at \texseans*{300910}},
  }
\end{chronos}
\begin{chronos}
  [
...
\end{chronos}
% \end{document}
% \def\bagpuss{Old, furry cat-puss}
\begin{chronos}
  [
%     dates=1800:2050,
    modern,
...
\end{chronos}
% \def\bagpuss{fat, furry cat-puss}
\begin{chronos}
  [
    sober judge,
...
%   \foreach \i in {-3000,-2900,...,-2000} \draw [red] (chronos year \i) ++(0pt,50pt) -- ++(0pt,-100pt);
\end{chronos}
% \Undefine\bagpuss
...
\begin{chronos}
  [
    dates=2008-02-03:2009-08-09,
    timeline show years=false,% dim ond newid y boolean, felly timeline@years yn cael ei awto-newid i on line ar y dechrau
  ]
\end{chronos}
\begin{chronos}
  [
    dates=2008-02-03:2009-08-09,
    timeline years=none,% osgoi style o gael ei awto-newid i on line ar y dechrau
  ]
\end{chronos}
\begin{chronos}
  [
    dates=2008-02-03:2009-08-09,
    timeline show years=true,
  ]
\end{chronos}
\begin{chronos}
  [
...
%     step from year={-3100},
    ...
    headings={paste bindings/chronos year 700/chronos year 1200,sewn bindings/chronos year 1400/chronos year 1800,ebooks/chronos year 1990/chronos year 2050},
...
%   }
  ...
  \chronosperiod{%
    ...
    xshift=-5pt,
    tag anchor=east,
%     /chronos/text content={something entirely different},
  ...
  \chronosperiod{%
    ...
    at=protoElamite use of cylinder seals |- u1.west,
    tag anchor=west,
    xshift=5pt,
    connectors=north,
    connectors=east,
...
\begin{chronos}
  ...
  \chronosperiod{dates=1000:1999,name={2\textsuperscript{nd} Millennium},as is,yshift=-2.5pt}
  \chronosperiod{dates=192:476,name=Eastern Roman Empire,yshift=-2.5pt}
  \chronosevent {date={1969},name=steps on the moon}
\end{chronos}
% \end{document}
...
%       lines on=overlay,
  ...
  \chronosevent {date={{-20}},name={Random event},line+={gray}}
  \chronosperiod {dates={{-5}}:{10},name=Random war,/chronos/llinell/.append style={fill=blue, blend mode=overlay}}
\end{chronos}
% \fi
% ateb: https://tex.stackexchange.com/a/325890/
\begin{chronos}
...
% The next set of \keyslabelname{} is conceptually similar to the previous three, but is used for generic settings which should be applied to \elementslabelname{}, rather than just those applicable to \conceptname{text tags}, as opposed to \conceptname{connections}, for example.
% \keyname{tag,tag+} should be used with caution; they should not be used for options which can be configured in other ways and are likely to do both more and less than you might expect\footnote{They should possibly be \verb|@tag| and \verb|@tag+|, but they make can be extremely useful when used correctly.
%   For example, they handle enabling and disabling colour rotation in a transparent way.}.
% 
% \DescribeKey{tag,tag+,tag''}\val{key-value list} \fortags{life,event,period,theory,info}\AfterLastParam
% \emph{Do not use this \keylabelname{} wantonly.}
% \meta{key-value list} to apply to this \elementlabelname{}.
% This is intended for corner cases and, possibly, in creating new \chronosstyleslabelname{}; it should \emph{not} be used for \TikZ{} or \pkg{chronos} \keyslabelname{} as they may not be processed correctly.
% \verb|tag| replaces any current list; \verb|tag+| adds to it.
% \emph{Note that \verb|tag| will overwrite various \pkg{chronos} settings, including whether colour rotation is enabled or disabled.}
% Do not use this \keylabelname{} to set options specific to the \conceptname[element](or){connection,line,text tag}.
% In most cases, you should simply give the \TikZ{} options rather than using this \keylabelname{} when creating particular \elementslabelname{}.
% In even more cases, you should not use this \keylabelname{} at all.
% For example,
% \begin{chronoscode}
% \chronosevent{%
%   scale=1.2,
% }
% \end{chronoscode}
% is simpler and more efficient than messing around with \verb|tag+|.
